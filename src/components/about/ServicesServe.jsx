...
import industyImage from "/home-content/industry.png"; 
 ...
 const ServicesServe = ({page}) => {
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const q = query(servicesRef, where("display", "array-contains", page));
  ...
  return (
    /* Scaled down padding from py-10 to py-8 */
    <section className="px-4 py-8 md:px-12 bg-white font-[Quicksand]">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="relative z-10 max-w-3xl w-full">
            {/* Scaled down font from text-6xl to text-3xl/4xl */}
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-1 tracking-tight">
              Our Services
            </h2>

            {/* DYNAMIC TITLE: Scaled down from text-3xl to text-xl/2xl */}
            ...
            {/* DESCRIPTION: Scaled down from text-md to text-sm/base */}
            ...
            {/* BUTTON: Scaled down padding from px-10 to px-8 */}
            <button
