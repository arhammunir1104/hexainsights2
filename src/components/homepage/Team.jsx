...
const Team = () => {
  ...
  // ✅ Use a ref so the interval never needs to restart just because hover state changed
  ...
  const handleScroll = () => {
    if (scrollRef.current && !isMoving.current) {
      ...
      setActiveIndex(prev =>
        newIndex !== prev && newIndex >= 0 && newIndex < teamMembers.length ? newIndex : prev
      );
    }
  };

  // ✅ Uses functional update — no stale activeIndex closure
  ...
  }, [teamMembers.length]); // ✅ Only re-creates if the member list size changes

  if (loading) return null;
  if (!teamMembers || teamMembers.length === 0) return null;

  return (
    <section className="relative w-full py-12 lg:py-20 bg-white overflow-hidden font-raleway">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-10">
          <h2 className="text-3xl lg:text-5xl  font-bold tracking-tight text-[#0037a5]">
        ...
        <div 
          className="relative"
          // ✅ Update the ref directly — no state change, no re-render, no interval restart
